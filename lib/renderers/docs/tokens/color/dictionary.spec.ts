...
import addColorToDictionary from './dictionary.js'

describe('addColorToDictionary', () => {
  const before = {}
  const token: DerefColorToken = { $type: 'color', $value: '#008800' }

  it('doesn\'t have a description if the token doesn\'t', () => {
    const actual = addColorToDictionary('color.green', token, before)
  ...
  it('adds the description to the dictionary', () => {
    ...
    const actual = addColorToDictionary('color.green', described, before)
  ...
  it('adds an SCSS variable if there is one', () => {
    ...
    const actual = addColorToDictionary('color.green', sassy, before)
