...
describe('renderHTML', () => {
  before(() => {
    mock({
      'html/src': {
        ...
        'swatches.html': '<!DOCTYPE html><html><head><title>Swatches Test</title><body><h1>Primary Colors</h1>{{ swatches.color.primary }}<h1>Secondary Colors</h1>{{ swatches.color.secondary }}</body></head></html>'
      },
      'html/dist': {}
    })
  })
  ...
  it('can render custom swatch sets', () => {
    ...
    renderHTML(list, { indir: 'html/src', outdir: 'html/dist', verbose: false })
    const actual = readFileSync('html/dist/swatches.html', { encoding: 'utf8' })
